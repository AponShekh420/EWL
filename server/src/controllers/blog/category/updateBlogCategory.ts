...
export const updateBlogCategory = async (
  ...
  try {
    ...
    const oldCategory = await BlogCategoryModel.findById(id);
    if (!oldCategory) return next(createError(404, "Blog category not found"));
    ...
    if (body.deletedImage) {
      deleteFileFromLocal(body.deletedImage, "blog-category");
    }
