...
const CourseOrderEmail = async (order) => {
    const courseRows = order.courses.map((course) => `
...
<td align="center">
$${(Number(course.price) * Number(course.quantity)).toFixed(2)}
</td>
...
`).join("");
    const packageRows = order.packages.map((pkg) => `
...
exports.default = CourseOrderEmail;
