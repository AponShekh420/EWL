          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {
              orders?.map((order: CourseOrderCardType, index: number) => {
                return (<CourseCard key={order.courses[0]._id._id + index} previewslug={`${order.courses[0]._id?.slug}`} title={order?.courses[0]?._id?.title || ""} slug={`${order.courses[0]._id?.slug}` || ""} thumbnail={order.courses[0]._id?.thumbnail || ""} speaker={order?.courses[0]?._id?.speaker}/>)
              })
            }
          </div>
        ) : (
