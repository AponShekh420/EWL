import { getClassOrderById } from "@/actions/classOrder";
...
import { ClassOrderType, OrderedClassType } from "@/types/ClassOrder";
...
export default async function OrderDetails({
  ...
  const { data: order }: { data: ClassOrderType } = await getClassOrderById(id);
  return (
    <div>
      <PageHeading
        ...
        breadcrumbList={[
          ...
          { name: "Orders", href: "/e-learning/class-orders" },
          {
            name: `#${id}`,
      ...
      >
        <Link href={`/dashboard/e-learning/class-orders/edit/${id}`}>
          <Button variant="blue">
      ...
      <div className="hidden sm:flex gap-4 flex-col sm:flex-row items-start sm:items-center text-sm font-lexend-deca text-gray-600 border-y py-4 my-5 ">
        ...
        <p className="sm:border-r w-fit sm:px-4">{order?.totalClass} Items</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.8fr_1fr] gap-8 lg:gap-4 xl:gap-8 2xl:gap-16">
        <div className=" mt-6 sm:mt-0">
          <Table className=" !rounded-md overflow-hidden">
            ...
            <TableBody>
              {order.classes?.map((classInfo: OrderedClassType, index: number) => (
                <TableRow key={classInfo?._id?.orderId || + index}>
                  <TableCell>
                    <Link href={`/shop/${classInfo?._id?.slug}`}>
                      <Image
                        src={getImageUrl(classInfo._id.thumbnail, "classes")}
                        width={100}
                        height={100}
                        alt={classInfo._id.title}
                        className="w-25 h-25 object-cover rounded-lg"
                      />
                      <h5 className="font-semibold mt-2">{classInfo._id.title}</h5>
                      <p className="text-sm text-gray-600">
                        {classInfo._id.category}
                      </p>
                    </Link>
                  </TableCell>
                  <TableCell>${classInfo.price}</TableCell>
                  <TableCell>{classInfo.quantity}</TableCell>
                  <TableCell className="font-medium">
                    ${Number(classInfo.price) * Number(classInfo.quantity)}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
