...
import { addCourseCheckoutField, resetCourseCheckoutFields } from "@/redux/features/checkout/courseCheckoutFormSlice";
import { BASE_URL } from "@/utils/envVariable";
import getCourseOrderRequestData from "@/lib/getCourseOrderRequestData";
...
export default function CheckoutForm() {
  ...
  const requestData = getCourseOrderRequestData(cart, checkoutForm)
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(BASE_URL + "/api/e-learning/courses/notify", {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        toast.success("Notification request sent successfully!");
        dispatch(resetCourseCheckoutFields());
        router.push("/courses"); // Redirect to courses page after successful submission
        // Handle success (e.g., show a success message, redirect, etc.)
      } else {
        console.log(data.errors)
        dispatch(addCourseCheckoutField({errors:  {
          ...data.errors,
        }}));
        setLoading(false);
        return;
      ...
      toast.error("Error sending notification request. Please try again.");
  ...
  return (
    <form action={BASE_URL + "/api/e-learning/order/create-order"} method="post" onSubmit={onSubmit} className="my-10  ">
      {/* billing address */}
      ...
      <TextBox
        ...
        placeholder="Notes about your e.g. special notes"
      ...
      <div className="mt-8">
        <Button
          type="submit"
          disabled={loading}
          className="w-full bg-teal hover:bg-teal/90 text-white h-12 rounded-lg disabled:bg-gray-400 disabled:cursor-not-allowed"
        >
