...
export default function BlogCategoriesTable({
  ...
  const deleteHandler = async (id: string) => {
    ...
    try {
      const res = await fetch(BASE_URL + "/api/blog/categories/" + id, {
        method: "DELETE",
      });
  ...
  const debouncedSearch = useMemo(
    () =>
      debounce((value: string) => {
        router.push(`/dashboard/blog-management/categories?search=${value}`);
      }, 500),
    [router],
  );
  return (
    <div>
      ...
      <Table className=" !rounded-md border">
        <TableCaption>A list of blog categories</TableCaption>
        <TableHeader className="bg-stone-100 ">
          <TableRow className="uppercase !h-13">
            ...
            <TableHead className="font-bold text-gray-500">Blog Count</TableHead>
            ...
            <TableRow key={category._id}>
              <TableCell>
                <div className="flex gap-4 items-center">
                  ...
                  <div className="flex items-center gap-x-4">
                    <Image
                      src={getImageUrl(category.thumbnail, "blog-category")}
                      alt={category.name}
                      width={50}
                      height={50}
              ...
              <TableCell>
                <div className="flex items-center gap-x-3">
                  <Link
                    href={`/dashboard/blog-management/categories/edit/${category.slug}`}
                  >
                    <Button
                      size="icon"
                      variant="outline"
