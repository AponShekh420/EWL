...
export default function ResourcesTable({
  ...
  const deleteHandler = async (id: string) => {
    ...
    try {
      const res = await fetch(
        BASE_URL + "/api/resources-management/resources/" + id,
        {
          method: "DELETE",
        },
      );
  ...
  const handleStatusChange = async (status: string, id: string) => {
    ...
    try {
      const res = await fetch(
        BASE_URL + "/api/resources-management/resources-status/" + id,
        {
          method: "PUT",
  ...
  const debouncedSearch = useMemo(
    () =>
      debounce((value: string) => {
        router.push(`/dashboard/resources?search=${value}`);
      }, 500),
    [router],
  );
    ...
    <div>
      ...
      <ScrollArea className="lg:w-[750px] xl:w-full overflow-x-scroll">
        <Table className=" !rounded-md border">
          <TableCaption>A list of resources posts</TableCaption>
              ...
              <TableRow key={resource._id}>
                ...
                <TableCell>
                  <div className="flex items-center gap-x-3">
                    <Link href={`/dashboard/resources/edit/${resource.slug}`}>
                      <Button
                        size="icon"
                        variant="outline"
                    ...
                    <DeleteModal
                      deleteAction={() => deleteHandler(resource._id)}
                    >
                      <Button
                        size="icon"
                        variant="outline"
