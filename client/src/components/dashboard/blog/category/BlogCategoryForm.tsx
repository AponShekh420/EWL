...
import { addBlogCategoryField, resetBlogCategoryFields, deleteExistingThumb } from "@/redux/features/blogCategory/blogCategoryFormSlice";
import { RootState } from "@/redux/store";
import { BlogCategoryType, BlogCategoryValidationErrors } from "@/types/BlogCategory";
...
export default function BlogCategoryForm({
  ...
  const {
    ...
    existingThumbnail,
    deletedImage,
  } = useSelector((state: RootState) => state.blogCategoryForm);
    ...
    try {
      ...
      if (path.includes("edit")) {
        ...
        const res = await fetch(
          BASE_URL + "/api/blog/categories/" + category?._id,
          {
            method: "PUT",
            body: formData,
          },
        ...
        if (!data.success) {
          ...
          dispatch(resetBlogCategoryFields());
          router.push("/dashboard/blog-management/categories");
          toast.success(data.message);
        }
      } else {
        const categoryPath =
          !categoryId || categoryId === "none" ? "category" : "subcategory";
        const res = await fetch(BASE_URL + "/api/blog/" + categoryPath, {
          method: "POST",
          body: formData,
        });
        const data = await res.json();
  ...
  useEffect(() => {
    fetch(BASE_URL + "/api/blog/categories", {cache: "no-store"})
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div>
      ...
      >
        <div>
          <h5 className="font-bold text-lg">Category For Blog</h5>
          <p className=" text-gray-700 mt-2">
            Create or Update category for different blog posts
          </p>
        </div>
        <div>
          <div className="grid sm:grid-cols-2 gap-8">
            <InputBox
              ...
              value={name}
              onChange={(e) =>
                dispatch(addBlogCategoryField({ name: e.target.value }))
              }
            ...
            <SelectBox
              ...
              onChange={(val) =>
                dispatch(addBlogCategoryField({ categoryId: val }))
              }
          ...
          <div>
            ...
            <DragAndDropFiles
              ...
              onFileChange={(files) => {
                if (files && files?.length > 0) {
                  dispatch(addBlogCategoryField({ thumbnail: files[0] }));
            ...
            {thumbnail && (
              <div className="relative w-fit">
                ...
                <button
                  ...
                  type="button"
                  onClick={() =>
                    dispatch(addBlogCategoryField({ thumbnail: null }))
                  }
                >
                  <Icon
            ...
            {existingThumbnail && (
              <div className="relative w-fit">
                <Image
                  src={getImageUrl(existingThumbnail, "blog-category")}
                  alt="preview"
                  width={220}
                  height={220}
                  className="rounded-md object-cover size-[220px]"
          ...
          <div>
            ...
            <Editor
              value={description}
              onChange={(val) =>
                dispatch(addBlogCategoryField({ description: val }))
              }
            />
            {errors.description && (
              <span className="text-red-500 text-xs mt-2 ml-1">
