...
export default function CheckoutDetails({ cart }: { cart: CourseCartType }) {
  ...
  const submitOrder= async(e: { preventDefault: () => void; })=>{
    ...
    const orderRes= await fetch(BASE_URL + "/api/e-learning/order/create-order",
      {
        method:"POST",
        headers:{
    ...
    if (!card) {
      await fetch(BASE_URL + "/api/e-learning/orders/" + data?.orderId,
        {
          method:"DELETE",
          headers:{
          "Content-Type": "application/json",
    ...
    if(result.paymentIntent.status=== "succeeded"){
      ...
      setTimeout(()=> {
        window.location.replace("/profile/my-courses")
      }, 3000)
    }
    setLoading(false);
  };
            ...
            {cart?.items.map((item, idx) => (
              <div key={idx} className="flex items-center gap-4 mb-2 relative">
                <div className="w-16 h-16 bg-[#E9ECEF] rounded-md flex items-center justify-center overflow-hidden flex-shrink-0 relative">
                  <Image
                    src={getImageUrl(item.course.thumbnail, "courses")}
                    alt="Keyboard"
